...
| 문서 번호 | BONGVIS |
| 버전 | 1.0 |
| 상태 | **검증됨** — 이 규격으로 쓴 문서 2건이 90점 통과 (2026-09-12) |
| 정본 | `bongvis/IDENTITY.md` (이 파일) — 정체성·원칙 |
| 실무 규칙 정본 | [`bongvis/HOUSE-STYLE.md`](HOUSE-STYLE.md) — 어떻게 쓰는가 (검증됨, 90점 통과 2건) |
| 최종 개정 | 2026-09-12 |

> **1.0 으로 올린 근거** — 0.1 은 "이 규격으로 쓴 문서가 0건"이라 초안이었다. 2026-09-12 에 이 규격으로 2건(플랫폼 비교 · 개념 설명서)을 실제로 써서 게시했고, 채점 루프(10점 → 80점 → 90점)를 거쳐 둘 다 90점 통과했다. 그 과정에서 나온 실무 규칙은 [`HOUSE-STYLE.md`](HOUSE-STYLE.md) 에, 이력은 `.bongvis/training.json` 에 있다.
...
| **1.0** | **2026-09-12** | **검증 완료.** 이 규격으로 쓴 문서 2건이 90점 통과. 채점 루프에서 나온 실무 규칙을 [`HOUSE-STYLE.md`](HOUSE-STYLE.md) 로 정본화하고, `templates/*.json` 의 `rules_from_training` 으로 집행하게 연결. 검증된 도구를 `scripts/` 에 보존 |
